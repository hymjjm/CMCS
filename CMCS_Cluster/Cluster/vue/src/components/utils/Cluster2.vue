...
// 使用辅助函数去重
  ...
  //  **************************表格下载*************************************************************************************
    downloadCSV() {
      const columnNames = ["Hugo_Symbol", "Chromosome", "Start_Position","GENOMIC_WT_ALLELE","GENOMIC_MUT_ALLELE","STRAND", "Cluster","ReferenceGenomes","mutDescription"]; // 指定列名
      const csvContent = "data:text/csv;charset=utf-8," +
        ...
        this.clusterData.flatMap(cluster => {
          return cluster.map(point => {
            ...
            // 将数据点属性按顺序组成一行
            return `${name},${chromosome},${start_position},${GENOMIC_WT_ALLELE},${GENOMIC_MUT_ALLELE},${STRAND},${clusterName},${ReferenceGenomes},${mutDescription}`;
          });
        }).join("\n");
