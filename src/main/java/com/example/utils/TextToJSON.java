...
public class TextToJSON {
    ...
    private static JSONObject convertTextToJson(String filename) throws JSONException {
...
//                links.put(link2);
    ...
    private static JSONObject getNode(JSONArray nodes, String nodeName) throws JSONException {
        for (int i = 0; i < nodes.length(); i++) {
            JSONObject node = nodes.getJSONObject(i);
    ...
    private static JSONArray sortNodes(JSONArray nodes) throws JSONException {
        JSONObject[] nodesArray = new JSONObject[nodes.length()];
        for (int i = 0; i < nodes.length(); i++) {
            nodesArray[i] = nodes.getJSONObject(i);
        }
        Arrays.sort(nodesArray, Comparator.comparingInt(a -> {
            try {
                return a.getInt("value");
            } catch (JSONException e) {
                e.printStackTrace();
            }
            return 0;
    ...
    private static void setNodePositions(JSONArray nodes) throws JSONException {
